...
/*
	...
	public static void setupEnv() {
		File originaFile = new File(TestSuite.PATH_TO_RESOURCES + "mutants/session/src/ODLExample.java");
		try {
			...
			allOperatorsSelected.add("ODL");
			allOperatorsSelected.add("AODU");
			MutationSystem.setJMutationPaths("ODLExample");
			TestSuite.setMutationSystemPathFor("ODLExample.java");
	...
	public void setup() {
		if (mujavaLogLines == null || mujavaLogLines.size() == 0) {
			mujavaLogLines = TestSuite
					.readLogFile("mutants/session/result/ODLExample/traditional_mutants/mutation_log");
		}
		...
		if (mutantsDirs == null || mutantsDirs.size() == 0) {
			mutantsDirs = TestSuite.listMutantsDirs("mutants/session/result/ODLExample/traditional_mutants");
		}
	}

	@Test
	public void test_ODL_x_AODU_Duplicated() {
...
//			return;
//		} else {
//			Assert.assertEquals(mutants.size(), 11);
//		}
