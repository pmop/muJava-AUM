...
public class MutantsGenPanel extends JPanel {
	...
	protected void initTripleColumnWidth(JTable table, AbstractTableModel model, int w1, int w2, int w3) {
	...
	protected void initColumnSizes(JTable table, AbstractTableModel model) {
		 ...
		 * table.getColumnModel().getColumn(i); switch(i) { case 0 :
		 * column.setMaxWidth(30); break; case 1 : column.setMaxWidth(90);
		 * break; case 2 : column.setMaxWidth(80); break; } }
		 */
	}
	...
	void runB_mouseClicked(MouseEvent e) {
		...
		if (file_list == null || file_list.length == 0) {
			System.err.println(
					"[ERROR] No class is selected. Please select one or more .java files for which you'd like to generate mutants.");
			return;
		}
		...
		// Leo:Adicionei um List para acumular todos operadores selecionados
		List<String> allOperatorsSelected = new ArrayList<String>();
		if (class_ops != null) {
			for (String op : class_ops) {
				allOperatorsSelected.add(op);
			}
		}
		if (traditional_ops != null) {
			for (String op : traditional_ops) {
				allOperatorsSelected.add(op);
			}
		}
		// -------------------------------------------
		...
		for (int i = 0; i < file_list.length; i++) {
			...
			try {
				...
				// having only one main method
					...
					System.out.println("Class " + file_name
				...
				// is selected
				if (class_ops != null) {
					cmGenEngine = new ClassMutantsGenerator(original_file, class_ops, allOperatorsSelected);
				...
				if (traditional_ops != null) {
					...
					// System.out.println("traditional_ops: " +
					// traditional_ops);
					tmGenEngine = new TraditionalMutantsGenerator(original_file, traditional_ops, allOperatorsSelected);
				...
				double diff = tDelta / 1000.0;
				System.out.println("[useless]Time: " + file_name + " - " + diff);
				List<String> lines = new ArrayList<String>();
				lines.add("[useless]Time: " + file_name + " - " + diff);
				try {
					LogReduction.logAppend(MutationSystem.MUTANT_HOME, "execution_time", lines);
