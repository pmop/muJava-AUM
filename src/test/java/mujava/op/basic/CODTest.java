...
/*
	...
	public static void setupEnv() {
		File originaFile = new File(TestSuite.PATH_TO_RESOURCES + "mutants/session/src/CODExample.java");
		try {
			...
			allOperatorsSelected.add("COD");
			allOperatorsSelected.add("ROR");
			MutationSystem.setJMutationPaths("CODExample");
			TestSuite.setMutationSystemPathFor("CODExample.java");
	...
	public void setup() {
		if (mujavaLogLines == null || mujavaLogLines.size() == 0) {
			mujavaLogLines = TestSuite
					.readLogFile("mutants/session/result/CODExample/traditional_mutants/mutation_log");
		}
		...
		if (mutantsDirs == null || mutantsDirs.size() == 0) {
			mutantsDirs = TestSuite.listMutantsDirs("mutants/session/result/CODExample/traditional_mutants");
		}
	}

	@Test
	public void test_COD_x_ROR_Duplicated() {
		...
		for (String line : mujavaLogLines) {
			...
			if (line.contains("void_f002(java.lang.Integer)")) {
				mutants.add(line.substring(0, line.indexOf(":")));
			}
			...
			if (line.contains("void_f006(boolean)")) {
				mutants.add(line.substring(0, line.indexOf(":")));
			}
		}
...
//			return;
//		} else {
//			Assert.assertEquals(mutants.size(), 6);
//		}
