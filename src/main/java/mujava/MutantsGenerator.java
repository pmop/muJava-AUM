...
/**
 * <p>
 * Generate mutants according to selected mutation operator(s) from
 * gui.GenMutantsMain. The original version is loaded, mutated, and compiled.
 * Outputs (mutated source and class files) are in the -mutants folder.
 * </p>
 * 
 * @author Yu-Seung Ma
 * @version 1.0
...
//	private void generateParseTree() throws OpenJavaException {
		...
		try {
			...
			for (int j = 0; j < typedecls.size(); ++j) {
				...
				if (class_decl.getName().equals(MutationSystem.CLASS_NAME)) {
					if (class_decl.isInterface() || class_decl.getModifiers().contains(ModifierList.ABSTRACT)) {
						return false;
					}
				}
	...
	private OJClass makeOJClass(Environment env, ClassDeclaration cdecl) {
		...
		try {
			Constructor constr = meta
					.getConstructor(new Class[] { Environment.class, OJClass.class, ClassDeclaration.class });
	...
	private static CompilationUnit parse(File file) throws OpenJavaException {
		...
		try {
			...
			System.err.println(
					"[ERROR] Unable to use the OpenJava Parser because the class version is unsupported. It may be that the openjava.jar file was compiled with a version of Java later than the one you're using.");
			System.err.println();
			e.printStackTrace();
			return null;
		}
		...
		try {
			...
			throw new OpenJavaException(" can't generate parse tree. Reason:" + e.getMessage());
		} catch (Exception e) {
			e.printStackTrace();
			result = null;
		}
	 ...
	 */
	private static String getMainClassName(FileEnvironment env, CompilationUnit comp_unit) throws ParseTreeException {
	...
	public void compileMutants() {
		...
		for (int i = 0; i < s.length; i++) {
			...
			try {
				...
				/*
				 * 12/19/13 Lin modified: if not in debug mode, for not showing
				 * the compile result when some mutants can't pass compiler if
				...
				if (Util.debug)
					...
					File tempCompileResultFile = new File(MutationSystem.SYSTEM_HOME + "/compile_output");
		...
		// System.out
		// .println("------------------------------------------------------------------");
		// System.out.println("Total mutants gnerated for " + fileName +": " +
	...
	private void compileOriginal() {
		String[] pars = { "-classpath", MutationSystem.CLASS_PATH,
