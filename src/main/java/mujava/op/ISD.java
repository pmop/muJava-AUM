...
/**
 * <p>
 * Generate ISI (Super keyword insertion) and ISD (Super keyword deletion)
 * mutants -- insert the <i>super</i> keyword; delete each occurrence of the
 * <i>super</i> keyword
 * </p>
 * 
 * @author Yu-Seung Ma
...
public class ISD extends mujava.op.util.Mutator {
	...
	public void visit(FieldAccess p) throws ParseTreeException {
		...
		if (ref_exp instanceof SelfAccess) {
			// Adicionei o metodo isDefinedField
			if (((SelfAccess) ref_exp).getAccessType() == SelfAccess.SUPER && !isEquivalent(p)) {
	...
	// By Leo. To avoid delete super from a FieldAccess that was not overriden
	boolean isDifinedField(FieldAccess p) {
		try {
			OJClass clazz = getSelfType();
			OJField[] ms = clazz.getDeclaredFields();

			for (int i = 0; i < ms.length; i++) {
				if (isSameField(ms[i], p))
					return true;
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	boolean isSameMethod(OJMethod m, MethodCall p) {
		try {
			...
			if (((elist != null) && (plist == null)) || ((elist == null) && (plist != null)))
				return false;
	...
	boolean isSameField(OJField m, FieldAccess p) {
		try {
			if (!(m.getName().equals(p.getName())))
				return false;

			if (!(m.getType().getName().equals(getType(p).getName())))
				return false;
	...
	public void visit(MethodCall p) throws ParseTreeException {
		...
		if (lexp != null) {
			if (lexp instanceof SelfAccess) {
				if ((((SelfAccess) lexp).getAccessType() == SelfAccess.SUPER) && !(occurRecursive(p))
						&& !isEquivalent(p)) {
					MethodCall mutant;
	...
	// Ele checa se o metodo nao vai gerar recursao
	 ...
	 * @return
	 */
	private boolean isEquivalent(MethodCall method) {
		if(!isDifinedMethod(method)){
			String desc = method.toFlattenString() + " => " + method.getName();
			logReduction("ISD", desc);
	...
	private boolean isEquivalent(FieldAccess field) {
		if(!isDifinedField(field)){
			String desc = field.toFlattenString() + " => " + field.getName();
			logReduction("ISD", desc);
			return LogReduction.AVOID;
		}
		return false;
