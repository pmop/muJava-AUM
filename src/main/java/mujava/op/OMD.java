...
/**
 * <p>
 * Generate OMD (Overloading method deletion) mutants -- delete each overloaded
 * method declaration, one at a time
 * </p>
 * 
 * @author Yu-Seung Ma
 * @version 1.0
...
public class OMD extends DeclAnalyzer {
	private final String[] primitives = { "char", "short", "int", "long", "unsigned", "unsigned long", "float",
			"double", "lond double" };
		...
		for (i = 0; i < m.length; i++) {
			if (checked[i]) {
				try {
					...
					if (!isEquivalent(original, m[compatable_method[i]])) {
	 ...
	 * @param mutant
	 */
	// public void outputToFile(CompilationUnit comp_unit,MethodDeclaration
	// original,OJClass[] mutant_pars){
	...
	public OMD(openjava.mop.Environment oj_param0, openjava.mop.OJClass oj_param1,
	 ...
	 * @return
	 */
	private boolean isEquivalent(MethodDeclaration method, OJMethod mutant) {
		//OMD: If method alredy call ist overload method, then 
		//   the mutant can be equivalent
		StatementList stmtList = method.getBody();
			...
			if (stmt instanceof ReturnStatement) {
				...
				if(exp instanceof MethodCall){
					...
					if(mc.getName().equals(name)){
						OJClass[] parameterTypes = mutant.getParameterTypes();
						//TODO Neeed continue with the verifications
						//Precisa verificar os tipos dos parâmetros.
					}
				}
