...
public abstract class Toolbox {
	 ...
	 * @see openjava.mop.OJClass
	 */
	public static final OJEnumConstant pickupEnumConstant(
		OJEnumConstant[] src_enumConstants,
		String name) {
		for (int i = 0; i < src_enumConstants.length; ++i) {
			if (name.equals(src_enumConstants[i].getName())) {
				return src_enumConstants[i];
			}
		}
		return null;
	}
	...
	public static final String nameToJavaClassName(String ojcname) {
		//if Generics <> found, skip the <> part and only return the class type before the <>
		if (!ojcname.endsWith("[]") && !(ojcname.indexOf("<") > 0) && !(ojcname.indexOf(">") >0))
			return ojcname;
		else if((ojcname.indexOf("<") > 0) && (ojcname.indexOf(">") >0) && !ojcname.endsWith("[]")){
			return ojcname.substring(0, ojcname.indexOf("<"));
		}
		else if((ojcname.indexOf("<") > 0) && (ojcname.indexOf(">") >0) && ojcname.endsWith("[]")){
			String temp = ojcname.substring(0, ojcname.indexOf("<"));
			String temp1 = ojcname.substring(ojcname.lastIndexOf(">") + 1, ojcname.length());
			ojcname = temp + temp1;
		}
		//System.out.println("ojcname: " + ojcname);
