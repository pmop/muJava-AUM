...
public class OJClass implements OJMember {
	...
	/**
	 * Added for type parameters Java 1.5, look up classes from the bottom Environment (ClosedEnvironment or FileEnvironment)to top Environment (GlobalEnvironment)
	 * @param name
	 * @param env
	 * @return
	 * @throws OJClassNotFoundException
	 */
	public static OJClass forName(String name, Environment env)
			throws OJClassNotFoundException {
			DebugOut.println("OJClass.forName(\"" + name.toString() + "\")");
			...
			result = env.lookupClass(name);
			if (result != null)
				return result;
			//System.out.println("OJClass forName: " + name+"; "+ isArrayName(name));
			if (isArrayName(name)) {
				OJClass component = forName(stripBrackets(name));
				result = new OJClass(component);
				//System.out.println("OJClass forName result: " + result);
				env.record(name, result);
				return result;
			}
	...
	public boolean isEnumeration() {
		return substance.isEnumeration();
	}
	...
	private static final OJEnumConstant pickupEnumConstant(
			OJEnumConstant[] src_enumConstants,
			String name) {
			return Toolbox.pickupEnumConstant(src_enumConstants, name);
		}
...
class OJClassSourceCode extends OJClassImp {
	...
	private Vector enumConstants = new Vector();
	...
	OJClassSourceCode(
		...
		/**
		 * if this class is an Enumeration type and it has Enumeration constants
		 * add them as class fields
		 */
		
		if(ptree.isEnumeration() == true){
			EnumConstantList enumConstantList = ptree.getEnumConstants();
			if(enumConstantList != null){
				for(int i = 0;i < enumConstantList.size();i++){
					EnumConstant eachEnum = enumConstantList.get(i);
					//System.out.println("OJClass OJClassSourceCode EnumConstant:" + eachEnum.getName() + ";type: " + eachEnum.getEnumType());
					//OJEnumConstant enumConstant = new OJEnumConstant(this.env, holder, eachEnum);
					OJField enumConstant = new OJField(this.env, holder, eachEnum);
					this.enumConstants.addElement(enumConstant);
					this.fields.addElement(enumConstant);
				}
			}
		}
		/**
		 * if(ptree.getBody() != null) is added on 09/20/2012
		 * when visiting enumeration types, it may not have a body, only several members
		 * e.g. public enum Season {Winter, Spring, Summer, Fall} this enumeration does not have any member declarations
		 * This is because EnumDeclaration is casted to ClassDeclaration 
		 */
		if(ptree.getBody() != null){
			...
			for (int i = 0, len = mdecls.size(); i < len; ++i) {
				...
				if (mdecl instanceof ClassDeclaration) {
					...
					//System.out.println("OJClass OJClassSourceCode FieldDeclaration: " + d.getName() + "; type: " + d.getTypeSpecifier());
	...
	OJEnumConstant[] getDeclaredEnumConstants() {
		OJEnumConstant[] result = new OJEnumConstant[enumConstants.size()];
		for (int i = 0; i < result.length; ++i) {
			result[i] = (OJEnumConstant) enumConstants.elementAt(i);
		}
		return result;
	}
