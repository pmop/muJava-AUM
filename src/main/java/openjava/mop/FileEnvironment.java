...
public class FileEnvironment extends Environment {
    ...
    public OJClass lookupClass(String name) {
        ...
        //System.out.println("FileEnvironment lookupClass: " + name +" " + clazz);
        if (clazz != null)
            return clazz;
        try {
	 ...
	 * Imports a class.
	 * 
	 * @param name the fully-qualified name of the imported class
	 *            
    ...
    public boolean importClass(String qualified_name) {
        ...
        /*	if(qualified_name.indexOf("static") >= 0){
        		System.out.println("qualified_name: " + qualified_name);
        		String[] s = qualified_name.split(" ");
        		System.out.println("s1: " + s[1]);
        		importedClasses.addElement(s[1]);
        	}
        	else{*/
    ...
    private String searchImportedClasses(String simple_name) {
    	String localName = simple_name;
    	//remove the content in the brackets such as java.util.List<? extends Number> 
    	if((simple_name.indexOf("<") > 0) && (simple_name.indexOf(">") >0)){
			String temp = simple_name.substring(0, simple_name.indexOf("<"));
			String temp1 = simple_name.substring(simple_name.lastIndexOf(">") + 1, simple_name.length());
			simple_name = temp + temp1;
		}
    	
        String tail = "." + simple_name;
        for (int i = 0, size = importedClasses.size(); i < size; ++i) {
            String qname = (String) importedClasses.elementAt(i);           
            //return the name having generics type parameters if it does
            if (qname.endsWith(tail) && localName.indexOf("<") > 0 && localName.indexOf(">") > 0)
                return qname.substring(0, qname.lastIndexOf(".") + 1) + localName;
            else if(qname.endsWith(tail))
            	return qname;
        }
        return null;
    }
    ...
    private String searchImportedPackages(String simple_name) {
        ...
        found = null;
        //System.out.println("searchImportedPackages");
