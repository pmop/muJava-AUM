...
public abstract class ScopeHandler extends EvaluationShuttle {
	...
	public ClassDeclaration evaluateDown(ClassDeclaration ptree)
		...
		push(env);
		//System.out.println("ClassDeclaration evaluateDown " + ptree.getName()+": "  + env.currentClassName() + ": " + env.getMemberClasses().size());
		return ptree;
	}
	...
	private void recordLocalEnum(EnumDeclaration ptree) {	
		String classname = ptree.getName();
		//System.out.println("recordLocalEnum: "+ classname);
		Environment outer_env = getEnvironment();
		String qname = outer_env.toQualifiedName(classname);
		//System.out.println("EnumDeclaration_toQualifiedNameForEnum: "+ qname);
		...
		try {
			...
			OJClass clazz = new OJClass(outer_env, out_clazz, new ClassDeclaration(ptree));
	...
	/**
	 * Added for EnumConstant Java 1.5
	 * An EnumConstant is like a member initializer
	 * 09/21/2012
	 */
	/*
	public EnumConstant evaluateDown(EnumConstant ptree)
			throws ParseTreeException {
			pushClosedEnvironment();
			return ptree;
	}
	
	public EnumConstantList evaluateDown(EnumConstantList ptree)
			throws ParseTreeException {
			pushClosedEnvironment();
			return ptree;
	}
	*/
	 ...
	 */
	public Statement evaluateDown(AssertStatement ptree)
			throws ParseTreeException {
			pushClosedEnvironment();
			return ptree;
		}
	 ...
	 */
	/*
	public EnumConstantList evaluateUp(EnumConstantList ptree)
			throws ParseTreeException {
			pop();
			return ptree;
	} */
	 ...
	 * Add support for AssertStatement
	 */
	public Statement evaluateUp(AssertStatement ptree) throws ParseTreeException {
		pop();
		return ptree;
	}
